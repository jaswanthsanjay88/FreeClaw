...
> * **DOMAINE OFFICIEL :** Le **SEUL** site officiel est **[freeclaw.io](https://freeclaw.io)**, et le site de l'entreprise est **[sipeed.com](https://sipeed.com)**.
...
Téléchargez le binaire pour votre plateforme depuis la page des [releases](https://github.com/sipeed/freeclaw/releases).
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous avez besoin d'accéder aux endpoints de santé ou d'exposer des ports, définissez `FREECLAW_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.
...
```bash
# Poser une question
docker compose -f docker/docker-compose.yml run --rm freeclaw-agent -m "Combien font 2+2 ?"
...
> [!TIP]
> Configurez votre clé API dans `~/.freeclaw/config.json`.
...
**2. Configurer** (`~/.freeclaw/config.json`)
...
```bash
freeclaw agent -m "Combien font 2+2 ?"
```
...
Fichier de configuration : `~/.freeclaw/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de freeclaw en tant que service système. Ces variables sont indépendantes et contrôlent différents chemins.

| Variable          | Description                                                                                                                             | Chemin par Défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `FREECLAW_CONFIG` | Remplace le chemin du fichier de configuration. Cela indique directement à freeclaw quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.freeclaw/config.json` |
| `FREECLAW_HOME`   | Remplace le répertoire racine des données freeclaw. Cela modifie l'emplacement par défaut du `workspace` et des autres répertoires de données.          | `~/.freeclaw`             |

**Exemples :**

```bash
# Exécuter freeclaw en utilisant un fichier de configuration spécifique
...
# Exécuter freeclaw avec toutes ses données stockées dans /opt/freeclaw
# La configuration sera chargée à partir du fichier par défaut ~/.freeclaw/config.json
# Le workspace sera créé dans /opt/freeclaw/workspace
...
PicoClaw stocke les données dans votre workspace configuré (par défaut : `~/.freeclaw/workspace`) :
...
|--------|------------|-------------|
| `workspace` | `~/.freeclaw/workspace` | Répertoire de travail de l'agent |
...
* `FREECLAW_HEARTBEAT_ENABLED=false` pour désactiver
* `FREECLAW_HEARTBEAT_INTERVAL=60` pour modifier l'intervalle
...
```bash
freeclaw agent -m "Bonjour, comment ça va ?"
```
...
```
> Exécutez `freeclaw auth login --provider anthropic` pour configurer les identifiants OAuth.
...
| ------------------------- | ------------------------------------- |
| `freeclaw onboard`        | Initialiser la configuration & le workspace |
| `freeclaw agent -m "..."` | Discuter avec l'agent                 |
| `freeclaw agent`          | Mode de discussion interactif         |
| `freeclaw gateway`        | Démarrer la passerelle                |
| `freeclaw status`         | Afficher le statut                    |
| `freeclaw cron list`      | Lister toutes les tâches planifiées   |
| `freeclaw cron add ...`   | Ajouter une tâche planifiée           |
...
Les tâches sont stockées dans `~/.freeclaw/workspace/cron/` et traitées automatiquement.
...
Ajoutez la clé dans `~/.freeclaw/config.json` si vous utilisez Brave :
...
Cela se produit lorsqu'une autre instance du bot est en cours d'exécution. Assurez-vous qu'un seul `freeclaw gateway` fonctionne à la fois.
