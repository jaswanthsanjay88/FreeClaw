...
> * **Warning:** freeclaw is in early development now and may have unresolved network security issues. Do not deploy to production environments before the v1.0 release.
> * **Note:** freeclaw has recently merged a lot of PRs, which may result in a larger memory footprint (10–20MB) in the latest versions. We plan to prioritize resource optimization as soon as the current feature set reaches a stable state.
...
Download the firmware for your platform from the [release](https://github.com/sipeed/freeclaw/releases) page.
...
**Raspberry Pi Zero 2 W:** Use the binary that matches your OS: 32-bit Raspberry Pi OS → `make build-linux-arm` (output: `build/freeclaw-linux-arm`); 64-bit → `make build-linux-arm64` (output: `build/freeclaw-linux-arm64`). Or run `make build-pi-zero` to build both.
...
> [!TIP]
> **Docker Users**: By default, the Gateway listens on `127.0.0.1` which is not accessible from the host. If you need to access the health endpoints or expose ports, set `FREECLAW_GATEWAY_HOST=0.0.0.0` in your environment or update `config.json`.
...
```bash
# Ask a question
docker compose -f docker/docker-compose.yml run --rm freeclaw-agent -m "What is 2+2?"
...
> [!TIP]
> Set your API key in `~/.freeclaw/config.json`.
...
**2. Configure** (`~/.freeclaw/config.json`)
...
Talk to your freeclaw through Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, or WeCom
...
If `session_store_path` is empty, the session is stored in `&lt;workspace&gt;/whatsapp/`. Run `freeclaw gateway`; on first run, scan the QR code printed in the terminal with WhatsApp → Linked Devices.
...
Config file: `~/.freeclaw/config.json`

### Environment Variables

You can override default paths using environment variables. This is useful for portable installations, containerized deployments, or running freeclaw as a system service. These variables are independent and control different paths.

| Variable          | Description                                                                                                                             | Default Path              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `FREECLAW_CONFIG` | Overrides the path to the configuration file. This directly tells freeclaw which `config.json` to load, ignoring all other locations. | `~/.freeclaw/config.json` |
| `FREECLAW_HOME`   | Overrides the root directory for freeclaw data. This changes the default location of the `workspace` and other data directories.          | `~/.freeclaw`             |

**Examples:**

```bash
# Run freeclaw using a specific config file
...
# Run freeclaw with all its data stored in /opt/freeclaw
# Config will be loaded from the default ~/.freeclaw/config.json
# Workspace will be created at /opt/freeclaw/workspace
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.freeclaw/workspace` | Working directory for the agent           |
...
* `FREECLAW_HEARTBEAT_ENABLED=false` to disable
* `FREECLAW_HEARTBEAT_INTERVAL=60` to change interval
...
> Run `freeclaw auth login --provider anthropic` to paste your API token.
...
| Command                   | Description                   |
| ------------------------- | ----------------------------- |
| `freeclaw onboard`        | Initialize config & workspace |
| `freeclaw agent -m "..."` | Chat with the agent           |
| `freeclaw agent`          | Interactive chat mode         |
| `freeclaw gateway`        | Start the gateway             |
| `freeclaw status`         | Show status                   |
| `freeclaw cron list`      | List all scheduled jobs       |
| `freeclaw cron add ...`   | Add a scheduled job           |
...
Jobs are stored in `~/.freeclaw/workspace/cron/` and processed automatically.

## 🤝 Contribute & Roadmap

PRs welcome! The codebase is intentionally small and readable. 🤗

See our full [Community Roadmap](https://github.com/sipeed/freeclaw/blob/main/ROADMAP.md).
...
Add the key to `~/.freeclaw/config.json` if using Brave:
...
This happens when another instance of the bot is running. Make sure only one `freeclaw gateway` is running at a time.
