...
> - **官方域名:** 唯一的官方网站是 **[freeclaw.io](https://freeclaw.io)**，公司官网是 **[sipeed.com](https://sipeed.com)**。
> - **警惕:** 许多 `.ai/.org/.com/.net/...` 后缀的域名被第三方抢注，请勿轻信。
> - **注意:** freeclaw正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在1.0正式版发布前，请不要将其部署到生产环境中
> - **注意:** freeclaw最近合并了大量PRs，近期版本可能内存占用较大(10~20MB)，我们将在功能较为收敛后进行资源占用优化.
...
freeclaw 可以将你10年前的老旧手机废物利用，变身成为你的AI助理！快速指南:
...
然后跟随下面的“快速开始”章节继续配置freeclaw即可使用！  
...
从 [Release 页面](https://github.com/sipeed/freeclaw/releases) 下载适用于您平台的固件。
...
> [!TIP]
> **Docker 用户**: 默认情况下, Gateway 监听 `127.0.0.1`，该端口不会暴露到容器外。如果需要通过端口映射访问健康检查接口，请在环境变量中设置 `FREECLAW_GATEWAY_HOST=0.0.0.0` 或修改 `config.json`。
...
```bash
# 提问
docker compose -f docker/docker-compose.yml run --rm freeclaw-agent -m "2+2 等于几？"
...
> [!TIP]
> 在 `~/.freeclaw/config.json` 中设置您的 API Key。
...
**2. 配置 (Configure)** (`~/.freeclaw/config.json`)
...
配置文件路径: `~/.freeclaw/config.json`

### 环境变量

你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 freeclaw 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `FREECLAW_CONFIG` | 覆盖配置文件的路径。这直接告诉 freeclaw 加载哪个 `config.json`，忽略所有其他位置。 | `~/.freeclaw/config.json` |
| `FREECLAW_HOME`   | 覆盖 freeclaw 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.freeclaw`             |

**示例：**

```bash
# 使用特定的配置文件运行 freeclaw
# 工作区路径将从该配置文件中读取
FREECLAW_CONFIG=/etc/freeclaw/production.json freeclaw gateway

# 在 /opt/freeclaw 中存储所有数据运行 freeclaw
# 配置将从默认的 ~/.freeclaw/config.json 加载
# 工作区将在 /opt/freeclaw/workspace 创建
...
PicoClaw 将数据存储在您配置的工作区中（默认：`~/.freeclaw/workspace`）：
...
1. `~/.freeclaw/workspace/skills`（工作区）
2. `~/.freeclaw/skills`（全局）
...
- `FREECLAW_HEARTBEAT_ENABLED=false` 禁用
- `FREECLAW_HEARTBEAT_INTERVAL=60` 更改间隔
...
> 运行 `freeclaw auth login --provider anthropic` 来设置 OAuth 凭证。
...
| 命令                      | 描述               |
| ------------------------- | ------------------ |
| `freeclaw onboard`        | 初始化配置和工作区 |
| `freeclaw agent -m "..."` | 与 Agent 对话      |
| `freeclaw agent`          | 交互式聊天模式     |
| `freeclaw gateway`        | 启动网关 (Gateway) |
| `freeclaw status`         | 显示状态           |
| `freeclaw cron list`      | 列出所有定时任务   |
| `freeclaw cron add ...`   | 添加定时任务       |
...
任务存储在 `~/.freeclaw/workspace/cron/` 中并自动处理。
...
这表示有另一个机器人实例正在运行。请确保同一时间只有一个 `freeclaw gateway` 进程在运行。
