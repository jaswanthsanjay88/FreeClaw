...
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[freeclaw.io](https://freeclaw.io)**, website công ty là **[sipeed.com](https://sipeed.com)**.
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/sipeed/freeclaw/releases).
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `FREECLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose -f docker/docker-compose.yml run --rm freeclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.freeclaw/config.json`.
...
**2. Cấu hình** (`~/.freeclaw/config.json`)
...
```bash
freeclaw agent -m "Xin chào, bạn là ai?"
```
...
File cấu hình: `~/.freeclaw/config.json`

### Biến môi trường

Bạn có thể ghi đè các đường dẫn mặc định bằng cách sử dụng các biến môi trường. Điều này hữu ích cho việc cài đặt di động, triển khai container hóa hoặc chạy freeclaw như một dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.

| Biến              | Mô tả                                                                                                                             | Đường dẫn mặc định        |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `FREECLAW_CONFIG` | Ghi đè đường dẫn đến file cấu hình. Điều này trực tiếp yêu cầu freeclaw tải file `config.json` nào, bỏ qua tất cả các vị trí khác. | `~/.freeclaw/config.json` |
| `FREECLAW_HOME`   | Ghi đè thư mục gốc cho dữ liệu freeclaw. Điều này thay đổi vị trí mặc định của `workspace` và các thư mục dữ liệu khác.          | `~/.freeclaw`             |

**Ví dụ:**

```bash
# Chạy freeclaw bằng một file cấu hình cụ thể
# Đường dẫn workspace sẽ được đọc từ trong file cấu hình đó
FREECLAW_CONFIG=/etc/freeclaw/production.json freeclaw gateway

# Chạy freeclaw với tất cả dữ liệu được lưu trữ trong /opt/freeclaw
# Cấu hình sẽ được tải từ ~/.freeclaw/config.json mặc định
# Workspace sẽ được tạo tại /opt/freeclaw/workspace
...
PicoClaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.freeclaw/workspace`):
...
|----------|---------|-------|
| `workspace` | `~/.freeclaw/workspace` | Thư mục làm việc của agent |
...
* `FREECLAW_HEARTBEAT_ENABLED=false` để tắt
* `FREECLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
```
> Chạy `freeclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.
...
| Lệnh | Mô tả |
| --- | --- |
| `freeclaw onboard` | Khởi tạo cấu hình & workspace |
| `freeclaw agent -m "..."` | Trò chuyện với agent |
| `freeclaw agent` | Chế độ chat tương tác |
| `freeclaw gateway` | Khởi động gateway (cho bot chat) |
| `freeclaw status` | Hiển thị trạng thái |
| `freeclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `freeclaw cron add ...` | Thêm tác vụ định kỳ |
...
Các tác vụ được lưu trong `~/.freeclaw/workspace/cron/` và được xử lý tự động.
...
Thêm key vào `~/.freeclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `freeclaw gateway` chạy tại một thời điểm.
