...
INTERNAL=github.com/sipeed/freeclaw/cmd/freeclaw/internal
...
WORKSPACE_DIR?=$(FREECLAW_HOME)/workspace
...
## build: Build the freeclaw binary for current platform
build: generate
...
## build-all: Build freeclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install freeclaw to system and copy builtin skills
install: build
...
## uninstall: Remove freeclaw from system
uninstall:
...
## uninstall-all: Remove freeclaw and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(FREECLAW_HOME)
	@echo "Removed workspace: $(FREECLAW_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build freeclaw-agent freeclaw-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build freeclaw-agent freeclaw-gateway
...
## docker-run: Run freeclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run freeclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run freeclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm freeclaw-agent

## docker-run-agent-full: Run freeclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm freeclaw-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi freeclaw:latest freeclaw:full 2>/dev/null || true

## help: Show this help message
help:
	@echo "freeclaw Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  make [target]"
	@echo ""
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.freeclaw/workspace)"
