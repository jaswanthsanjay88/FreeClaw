...
func TestGetAssistantName_Default(t *testing.T) {
	cfg := DefaultConfig()
	if got := cfg.GetAssistantName(); got != "Freeclaw" {
		t.Fatalf("GetAssistantName() = %q, want %q", got, "Freeclaw")
	}
}

func TestGetAssistantName_Custom(t *testing.T) {
	cfg := DefaultConfig()
	cfg.AssistantName = "MyCustomBot"
	if got := cfg.GetAssistantName(); got != "MyCustomBot" {
		t.Fatalf("GetAssistantName() = %q, want %q", got, "MyCustomBot")
	}
}
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	t.Setenv("FREECLAW_HOME", "")
	// Set a known home for consistent test results
	t.Setenv("HOME", "/tmp/home")

	cfg := DefaultConfig()
	want := filepath.Join("/tmp/home", ".freeclaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithPicoclawHome(t *testing.T) {
	t.Setenv("FREECLAW_HOME", "/custom/freeclaw/home")

	cfg := DefaultConfig()
	want := "/custom/freeclaw/home/workspace"

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with FREECLAW_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
