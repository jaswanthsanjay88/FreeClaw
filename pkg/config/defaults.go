...
func DefaultConfig() *Config {
	...
	// Priority: $FREECLAW_HOME > ~/.freeclaw
	var homePath string
	if freeclawHome := os.Getenv("FREECLAW_HOME"); freeclawHome != "" {
		homePath = freeclawHome
	} else {
		userHome, _ := os.UserHomeDir()
		homePath = filepath.Join(userHome, ".freeclaw")
	}
	workspacePath := filepath.Join(homePath, "workspace")

	return &Config{
		AssistantName: "Freeclaw",
		Agents: AgentsConfig{
		...
		Channels: ChannelsConfig{
			...
			WAReplyMate: WAReplyMateConfig{
				Enabled:              false,
				APIBaseURL:           "http://localhost:8080/api",
				MessagesDBPath:       "",
				PollIntervalSeconds:  1,
				StartupCatchupRows:   25,
				StartupCatchupMaxAge: 120,
				AutoStartBridge:      true,
				BridgeCommand:        "",
				BridgeArgs:           nil,
				BridgeWorkDir:        "",
				BridgeStartupTimeout: 20,
				OwnerJIDs:            FlexibleStringSlice{},
				NonOwnerDelaySeconds: 60,
				OwnerActiveWindowSec: 70,
				NonOwnerNoTools:      true,
				NonOwnerContextHours: 6,
				AllowFrom:            FlexibleStringSlice{},
			},
