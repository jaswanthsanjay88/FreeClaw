...
import (
	...
	"github.com/sipeed/freeclaw/pkg/skills"
)
...
func getGlobalConfigDir() string {
	if home := os.Getenv("FREECLAW_HOME"); home != "" {
		return home
	}
	...
	return filepath.Join(home, ".freeclaw")
}

func NewContextBuilder(workspace string, assistantName ...string) *ContextBuilder {
	// builtin skills: skills directory in current project
	// Use the skills/ directory under the current working directory
	builtinSkillsDir := strings.TrimSpace(os.Getenv("FREECLAW_BUILTIN_SKILLS"))
	if builtinSkillsDir == "" {
		wd, _ := os.Getwd()
	...
	if len(assistantName) > 0 {
		if n := strings.TrimSpace(assistantName[0]); n != "" {
			name = n
		}
	}
...
1. **Use tools only when needed** - Use tools for real actions (schedule reminders, send messages, execute commands, read files, web lookups). For normal conversation, answer directly without unnecessary tool calls.

2. **Never fake execution** - If an action is requested, either call the right tool or clearly explain what is missing.
