...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.freeclaw/workspace" {
		t.Errorf("expected workspace '~/.freeclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToPicoClaw(t *testing.T) {
	...
	freeCfg, warnings, err := cfg.ConvertToPicoClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}

	if freeCfg.Agents.Defaults.ModelName != "claude-sonnet-4-20250514" {
		t.Errorf("expected model 'claude-sonnet-4-20250514', got '%s'", freeCfg.Agents.Defaults.ModelName)
	}
	if freeCfg.Agents.Defaults.Workspace != "~/.freeclaw/workspace" {
		t.Errorf("expected workspace '~/.freeclaw/workspace', got '%s'", freeCfg.Agents.Defaults.Workspace)
	}

	if len(freeCfg.Agents.List) != 2 {
		t.Errorf("expected 2 agents, got %d", len(freeCfg.Agents.List))
	}
	if freeCfg.Agents.List[0].ID != "main" {
		t.Errorf("expected first agent id 'main', got '%s'", freeCfg.Agents.List[0].ID)
	}
	if freeCfg.Agents.List[1].Skills == nil || len(freeCfg.Agents.List[1].Skills) != 2 {
		t.Errorf("expected 2 skills for assistant agent")
	}

	if !freeCfg.Channels.Telegram.Enabled {
		t.Error("telegram should be enabled")
	}
	if freeCfg.Channels.Telegram.Token != "test-token" {
		t.Errorf("expected telegram token 'test-token', got '%s'", freeCfg.Channels.Telegram.Token)
	}

	if freeCfg.Channels.WhatsApp.BridgeURL != "http://localhost:3000" {
		t.Errorf("expected whatsapp bridge URL 'http://localhost:3000', got '%s'", freeCfg.Channels.WhatsApp.BridgeURL)
	}

	if freeCfg.Channels.Feishu.AppID != "app-id" {
		t.Errorf("expected feishu app ID 'app-id', got '%s'", freeCfg.Channels.Feishu.AppID)
	}

	if len(freeCfg.ModelList) != 1 {
		t.Errorf("expected 1 model config (no models.json provided), got %d", len(freeCfg.ModelList))
	}
...
func TestConvertToPicoClawWithQQAndDingTalk(t *testing.T) {
	...
	freeCfg, _, err := cfg.ConvertToPicoClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}

	if !freeCfg.Channels.QQ.Enabled {
		t.Error("qq should be enabled")
	}
	if freeCfg.Channels.QQ.AppID != "qq-app-id" {
		t.Errorf("expected qq app ID 'qq-app-id', got '%s'", freeCfg.Channels.QQ.AppID)
	}

	if !freeCfg.Channels.DingTalk.Enabled {
		t.Error("dingtalk should be enabled")
	}
	if freeCfg.Channels.DingTalk.ClientID != "ding-app-id" {
		t.Errorf("expected dingtalk client ID 'ding-app-id', got '%s'", freeCfg.Channels.DingTalk.ClientID)
	}

	if !freeCfg.Channels.MaixCam.Enabled {
		t.Error("maixcam should be enabled")
	}
	if freeCfg.Channels.MaixCam.Host != "192.168.1.100" {
		t.Errorf("expected maixcam host '192.168.1.100', got '%s'", freeCfg.Channels.MaixCam.Host)
	}
	if freeCfg.Channels.MaixCam.Port != 9000 {
		t.Errorf("expected maixcam port 9000, got %d", freeCfg.Channels.MaixCam.Port)
	}

	if !freeCfg.Channels.Slack.Enabled {
		t.Error("slack should be enabled")
	}
	if freeCfg.Channels.Slack.BotToken != "xoxb-test" {
		t.Errorf("expected slack bot token 'xoxb-test', got '%s'", freeCfg.Channels.Slack.BotToken)
	}
	if freeCfg.Channels.Slack.AppToken != "xapp-test" {
		t.Errorf("expected slack app token 'xapp-test', got '%s'", freeCfg.Channels.Slack.AppToken)
	}
}
...
func TestConvertToPicoClawWithMatrix(t *testing.T) {
	...
	if !freeCfg.Channels.Matrix.Enabled {
		t.Error("matrix should be enabled")
	}
	if freeCfg.Channels.Matrix.Homeserver != "https://matrix.example.com" {
		t.Errorf("expected matrix homeserver, got %q", freeCfg.Channels.Matrix.Homeserver)
	}
	if freeCfg.Channels.Matrix.UserID != "@bot:matrix.example.com" {
		t.Errorf("expected matrix user_id, got %q", freeCfg.Channels.Matrix.UserID)
	}
	if freeCfg.Channels.Matrix.AccessToken != "syt_test_token" {
		t.Errorf("expected matrix access_token, got %q", freeCfg.Channels.Matrix.AccessToken)
	}
	if len(freeCfg.Channels.Matrix.AllowFrom) != 1 ||
		freeCfg.Channels.Matrix.AllowFrom[0] != "@alice:matrix.example.com" {
		t.Errorf("unexpected matrix allow_from: %#v", freeCfg.Channels.Matrix.AllowFrom)
	}
...
func TestToStandardConfig(t *testing.T) {
	...
	stdCfg := freeCfg.ToStandardConfig()
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.freeclaw/workspace" {
		t.Errorf("expected workspace '~/.freeclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
