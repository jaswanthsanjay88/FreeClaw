...
import (
	...
	"github.com/sipeed/freeclaw/pkg/migrate/internal"
)
...
func (o *OpenclawHandler) ExecuteConfigMigration(srcConfigPath, dstConfigPath string) error {
	...
	freeCfg, warnings, err := openclawCfg.ConvertToPicoClaw(o.opts.SourceHome)
	if err != nil {
		return err
	}
	...
	incoming := freeCfg.ToStandardConfig()
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".freeclaw", 1)
	return path
}
