...
// PruneInactive removes sessions that have not been updated within maxAge.
// It returns the number of removed sessions.
func (sm *SessionManager) PruneInactive(maxAge time.Duration) (int, error) {
	if maxAge <= 0 {
		return 0, nil
	}
	...
	for key, s := range sm.sessions {
		...
		removed = append(removed, removal{key: key, filename: sanitizeFilename(key)})
		delete(sm.sessions, key)
	}
	sm.mu.Unlock()

	if len(removed) == 0 || sm.storage == "" {
		return len(removed), nil
	}

	var firstErr error
	for _, item := range removed {
		sessionPath := filepath.Join(sm.storage, item.filename+".json")
