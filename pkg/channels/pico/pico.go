...
// freeConn represents a single WebSocket connection.
...
func (pc *freeConn) writeJSON(v any) error {
	if pc.closed.Load() {
...
type PicoChannel struct {
	*channels.BaseChannel
	...
	connections sync.Map // connID → *freeConn
...
func NewPicoChannel(cfg config.PicoConfig, messageBus *bus.MessageBus) (*PicoChannel, error) {
	if cfg.Token == "" {
		return nil, fmt.Errorf("free token is required")
	}

	base := channels.NewBaseChannel("free", cfg, messageBus, cfg.AllowFrom)
...
func (c *PicoChannel) Start(ctx context.Context) error {
	logger.InfoC("free", "Starting Pico Protocol channel")
	c.ctx, c.cancel = context.WithCancel(ctx)
	c.SetRunning(true)
	logger.InfoC("free", "Pico Protocol channel started")
	return nil
}

// Stop implements Channel.
func (c *PicoChannel) Stop(ctx context.Context) error {
	logger.InfoC("free", "Stopping Pico Protocol channel")
	c.SetRunning(false)

	// Close all connections
	c.connections.Range(func(key, value any) bool {
		if pc, ok := value.(*freeConn); ok {
			pc.close()
		}
	...
	logger.InfoC("free", "Pico Protocol channel stopped")
	return nil
}

// WebhookPath implements channels.WebhookHandler.
func (c *PicoChannel) WebhookPath() string { return "/free/" }

// ServeHTTP implements http.Handler for the shared HTTP server.
func (c *PicoChannel) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	path := strings.TrimPrefix(r.URL.Path, "/free")
...
func (c *PicoChannel) broadcastToSession(chatID string, msg PicoMessage) error {
	// chatID format: "free:<sessionID>"
	sessionID := strings.TrimPrefix(chatID, "free:")
	msg.SessionID = sessionID

	var sent bool
	c.connections.Range(func(key, value any) bool {
		pc, ok := value.(*freeConn)
		if !ok {
			return true
		}
		if pc.sessionID == sessionID {
			if err := pc.writeJSON(msg); err != nil {
				logger.DebugCF("free", "Write to connection failed", map[string]any{
					"conn_id": pc.id,
					"error":   err.Error(),
				})
			} else {
...
func (c *PicoChannel) handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.ErrorCF("free", "WebSocket upgrade failed", map[string]any{
			"error": err.Error(),
		})
		return
	}
	...
	logger.InfoCF("free", "WebSocket client connected", map[string]any{
		"conn_id":    pc.id,
...
func (c *PicoChannel) readLoop(pc *freeConn) {
	defer func() {
		pc.close()
		c.connections.Delete(pc.id)
		c.connCount.Add(-1)
		logger.InfoCF("free", "WebSocket client disconnected", map[string]any{
			"conn_id":    pc.id,
	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseNormalClosure) {
				logger.DebugCF("free", "WebSocket read error", map[string]any{
					"conn_id": pc.id,
					"error":   err.Error(),
				})
			}
...
func (c *PicoChannel) pingLoop(pc *freeConn, interval time.Duration) {
...
func (c *PicoChannel) handleMessage(pc *freeConn, msg PicoMessage) {
	switch msg.Type {
	case TypePing:
...
func (c *PicoChannel) handleMessageSend(pc *freeConn, msg PicoMessage) {
	...
	chatID := "free:" + sessionID
	senderID := "free-user"

	peer := bus.Peer{Kind: "direct", ID: "free:" + sessionID}
	...
	logger.DebugCF("free", "Received message", map[string]any{
		"session_id": sessionID,
	...
	sender := bus.SenderInfo{
		Platform:    "free",
		PlatformID:  senderID,
		CanonicalID: identity.BuildCanonicalID("free", senderID),
	}
