...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("freeclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@freeclaw hello", want: true},
		{text: "hi @freeclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bfreeclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@freeclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@freeclaw:matrix.org hello", want: "hello"},
		{in: "@freeclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@freeclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@freeclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@freeclaw:matrix.org">PicoClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40freeclaw%3Amatrix.org">PicoClaw</a> hello`,
			},
			want: true,
		},
	}
