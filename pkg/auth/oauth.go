...
func GoogleAntigravityOAuthConfig() OAuthProviderConfig {
	// Keep credentials out of source to avoid accidental secret leakage.
	clientID := strings.TrimSpace(os.Getenv("FREECLAW_GOOGLE_OAUTH_CLIENT_ID"))
	clientSecret := strings.TrimSpace(os.Getenv("FREECLAW_GOOGLE_OAUTH_CLIENT_SECRET"))
	return OAuthProviderConfig{
...
func buildAuthorizeURL(cfg OAuthProviderConfig, pkce PKCECodes, state, redirectURI string) string {
	...
	if isGoogle {
		...
		if strings.Contains(strings.ToLower(cfg.Issuer), "auth.openai.com") {
			params.Set("originator", "freeclaw")
		}
		if cfg.Originator != "" {
