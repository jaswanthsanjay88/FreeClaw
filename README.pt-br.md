...
<h3>Hardware de $10 · 10MB de RAM · Boot em 1s · 皮皮虾，我们走！</h3>
  ...
  <p>
    ...
    <br>
    <a href="https://freeclaw.io"><img src="https://img.shields.io/badge/Website-freeclaw.io-blue?style=flat&logo=google-chrome&logoColor=white" alt="Website"></a>
...
<p align="center">
<img src="assets/freeclaw_mem.gif" width="360" height="240">
</p>
</td>
<td align="center" valign="top">
<p align="center">
...
> * **DOMÍNIO OFICIAL:** O **ÚNICO** site oficial é o **[freeclaw.io](https://freeclaw.io)**, e o site da empresa é o **[sipeed.com](https://sipeed.com)**.
...
<tr>
<td align="center"><p align="center"><img src="assets/freeclaw_code.gif" width="240" height="180"></p></td>
<td align="center"><p align="center"><img src="assets/freeclaw_memory.gif" width="240" height="180"></p></td>
<td align="center"><p align="center"><img src="assets/freeclaw_search.gif" width="240" height="180"></p></td>
</tr>
<tr>
...
wget https://github.com/sipeed/freeclaw/releases/download/v0.1.1/freeclaw-linux-arm64
chmod +x freeclaw-linux-arm64
pkg install proot
termux-chroot ./freeclaw-linux-arm64 onboard
```
...
Baixe o binário para sua plataforma na página de [releases](https://github.com/sipeed/freeclaw/releases).

### Instalar a partir do código-fonte (funcionalidades mais recentes, recomendado para desenvolvimento)

```bash
git clone https://github.com/sipeed/freeclaw.git
...
> [!TIP]
> **Usuários Docker**: Por padrão, o Gateway ouve em `127.0.0.1`, o que não é acessível a partir do host. Se você precisar acessar os endpoints de integridade ou expor portas, defina `FREECLAW_GATEWAY_HOST=0.0.0.0` em seu ambiente ou atualize o `config.json`.

```bash
# 5. Ver logs
docker compose -f docker/docker-compose.yml logs -f freeclaw-gateway
...
```bash
# Fazer uma pergunta
docker compose -f docker/docker-compose.yml run --rm freeclaw-agent -m "Quanto e 2+2?"

# Modo interativo
docker compose -f docker/docker-compose.yml run --rm freeclaw-agent
```
...
> [!TIP]
> Configure sua API key em `~/.freeclaw/config.json`.
...
**2. Configurar** (`~/.freeclaw/config.json`)
...
> **Nota**: O WeCom App (callbacks de webhook) é servido pelo Gateway compartilhado (padrão 127.0.0.1:18790). Em produção use um proxy reverso HTTPS para expor a porta do Gateway, ou atualize `FREECLAW_GATEWAY_HOST` para `0.0.0.0` se necessário.
...
Arquivo de configuração: `~/.freeclaw/config.json`

### Variáveis de Ambiente

Você pode substituir os caminhos padrão usando variáveis de ambiente. Isso é útil para instalações portáteis, implantações em contêineres ou para executar o freeclaw como um serviço do sistema. Essas variáveis são independentes e controlam caminhos diferentes.

| Variável          | Descrição                                                                                                                             | Caminho Padrão            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `FREECLAW_CONFIG` | Substitui o caminho para o arquivo de configuração. Isso informa diretamente ao freeclaw qual `config.json` carregar, ignorando todos os outros locais. | `~/.freeclaw/config.json` |
| `FREECLAW_HOME`   | Substitui o diretório raiz dos dados do freeclaw. Isso altera o local padrão do `workspace` e de outros diretórios de dados.          | `~/.freeclaw`             |

**Exemplos:**

```bash
# Executar o freeclaw usando um arquivo de configuração específico
# O caminho do workspace será lido de dentro desse arquivo de configuração
FREECLAW_CONFIG=/etc/freeclaw/production.json freeclaw gateway

# Executar o freeclaw com todos os seus dados armazenados em /opt/freeclaw
# A configuração será carregada do ~/.freeclaw/config.json padrão
# O workspace será criado em /opt/freeclaw/workspace
FREECLAW_HOME=/opt/freeclaw freeclaw agent

# Use ambos para uma configuração totalmente personalizada
FREECLAW_HOME=/srv/freeclaw FREECLAW_CONFIG=/srv/freeclaw/main.json freeclaw gateway
```

### Estrutura do Workspace

O PicoClaw armazena dados no workspace configurado (padrão: `~/.freeclaw/workspace`):
...
|-------|--------|-----------|
| `workspace` | `~/.freeclaw/workspace` | Diretório de trabalho do agente |
...
```bash
export FREECLAW_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
* `FREECLAW_HEARTBEAT_ENABLED=false` para desabilitar
* `FREECLAW_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
```
> Execute `freeclaw auth login --provider anthropic` para configurar credenciais OAuth.
...
| Comando | Descrição |
| --- | --- |
| `freeclaw onboard` | Inicializar configuração & workspace |
| `freeclaw agent -m "..."` | Conversar com o agente |
| `freeclaw agent` | Modo de chat interativo |
| `freeclaw gateway` | Iniciar o gateway (para bots de chat) |
| `freeclaw status` | Mostrar status |
| `freeclaw cron list` | Listar todas as tarefas agendadas |
| `freeclaw cron add ...` | Adicionar uma tarefa agendada |
...
As tarefas são armazenadas em `~/.freeclaw/workspace/cron/` e processadas automaticamente.
...
Adicione a key em `~/.freeclaw/config.json` se usar o Brave:
...
Isso acontece quando outra instância do bot está em execução. Certifique-se de que apenas um `freeclaw gateway` esteja rodando por vez.
