...
[リリースページ](https://github.com/sipeed/freeclaw/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `FREECLAW_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
> [!TIP]
> `~/.freeclaw/config.json` に API キーを設定してください。
...
> **Docker Compose**: Gateway HTTP サーバーは共有の `127.0.0.1:18790` で Webhook を提供します。ホストからアクセスするには `freeclaw-gateway` サービスに `ports: ["18790:18790"]` を追加してください。
...
設定ファイル: `~/.freeclaw/config.json`

### 環境変数

環境変数を使用してデフォルトのパスを上書きできます。これは、ポータブルインストール、コンテナ化されたデプロイメント、または freeclaw をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `FREECLAW_CONFIG` | 設定ファイルへのパスを上書きします。これにより、freeclaw は他のすべての場所を無視して、指定された `config.json` をロードします。 | `~/.freeclaw/config.json` |
| `FREECLAW_HOME`   | freeclaw データのルートディレクトリを上書きします。これにより、`workspace` やその他のデータディレクトリのデフォルトの場所が変更されます。          | `~/.freeclaw`             |

**例：**

```bash
# 特定の設定ファイルを使用して freeclaw を実行する
# ワークスペースのパスはその設定ファイル内から読み込まれます
FREECLAW_CONFIG=/etc/freeclaw/production.json freeclaw gateway

# すべてのデータを /opt/freeclaw に保存して freeclaw を実行する
# 設定はデフォルトの ~/.freeclaw/config.json からロードされます
# ワークスペースは /opt/freeclaw/workspace に作成されます
...
PicoClaw は設定されたワークスペース（デフォルト: `~/.freeclaw/workspace`）にデータを保存します：
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.freeclaw/workspace` | エージェントの作業ディレクトリ |
...
**環境変数:**
- `FREECLAW_HEARTBEAT_ENABLED=false` で無効化
- `FREECLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
```
> OAuth認証を設定するには、`freeclaw auth login --provider anthropic` を実行してください。
...
| コマンド | 説明 |
|---------|------|
| `freeclaw onboard` | 設定＆ワークスペースの初期化 |
| `freeclaw agent -m "..."` | エージェントとチャット |
| `freeclaw agent` | インタラクティブチャットモード |
| `freeclaw gateway` | ゲートウェイを起動 |
| `freeclaw status` | ステータスを表示 |
...
別のインスタンスが実行中の場合に発生します。`freeclaw gateway` が 1 つだけ実行されていることを確認してください。
