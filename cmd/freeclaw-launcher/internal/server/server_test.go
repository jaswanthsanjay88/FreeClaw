...
func TestGetConfig_MissingFile_ReturnsDefault(t *testing.T) {
	mux := http.NewServeMux()
	RegisterConfigAPI(mux, "/tmp/nonexistent-freeclaw-launcher-test/config.json")
...
func TestDefaultConfigPath(t *testing.T) {
	...
	if !strings.HasSuffix(path, filepath.Join(".freeclaw", "config.json")) {
		t.Errorf("expected path ending with .freeclaw/config.json, got %q", path)
	}
}
