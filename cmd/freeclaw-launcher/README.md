...
```bash
# Build
go build -o freeclaw-launcher ./cmd/freeclaw-launcher/

# Run with default config path (~/.freeclaw/config.json)
./freeclaw-launcher

# Specify a config file
./freeclaw-launcher ./config.json

# Allow LAN access
./freeclaw-launcher -public
```
...
```
Usage: freeclaw-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.freeclaw/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.freeclaw/config.json"
}
```
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the freeclaw-config UI.
...
Gets the running status of the `freeclaw gateway` process.
...
Starts the `freeclaw gateway` process in the background.
...
Stops the running `freeclaw gateway` process.
...
```bash
go test -v ./cmd/freeclaw-launcher/
```
