...
# 运行（使用默认配置路径 ~/.freeclaw/config.json）
./freeclaw-launcher
...
```
Usage: freeclaw-launcher [options] [config.json]

Arguments:
  config.json    配置文件路径（默认: ~/.freeclaw/config.json）
...
认证完成后 Google 回调至 `GET /auth/callback`，自动保存凭据并重定向回 freeclaw-config 页面。
