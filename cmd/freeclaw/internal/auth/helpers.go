...
import (
	...
	"github.com/sipeed/freeclaw/pkg/auth"
...
func authLoginGoogleAntigravity() error {
	...
	fmt.Println("Try it: freeclaw agent -m \"Hello world\"")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: freeclaw auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: freeclaw auth login --provider google-antigravity",
		)
	}
