...
import (
	...
	"github.com/sipeed/freeclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to freeclaw",
		Args:  cobra.NoArgs,
		Example: `  freeclaw migrate
  freeclaw migrate --from openclaw
  freeclaw migrate --dry-run
  freeclaw migrate --refresh
  freeclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.freeclaw)")
