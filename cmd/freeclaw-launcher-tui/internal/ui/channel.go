...
func (s *appState) buildChannelMenuItems() []MenuItem {
	return []MenuItem{
		...
		channelItem(
			"WAReplyMate",
			"WAReplyMate module bridge",
			s.config.Channels.WAReplyMate.Enabled,
			func() { s.push("channel-wareplymate", s.wareplymateForm()) },
		),
		channelItem(
			"Feishu",
			"Feishu bot settings",
...
func (s *appState) wareplymateForm() tview.Primitive {
	cfg := &s.config.Channels.WAReplyMate
	form := baseChannelForm("WAReplyMate", cfg.Enabled, s.makeChannelOnEnabled(&cfg.Enabled))
	form.AddInputField("API Base URL", cfg.APIBaseURL, 128, nil, func(text string) {
		cfg.APIBaseURL = strings.TrimSpace(text)
	})
	form.AddInputField("Messages DB Path", cfg.MessagesDBPath, 256, nil, func(text string) {
		cfg.MessagesDBPath = strings.TrimSpace(text)
	})
	addIntField(form, "Poll Interval (sec)", cfg.PollIntervalSeconds, func(value int) {
		cfg.PollIntervalSeconds = value
	})
	form.AddCheckbox("Auto Start Bridge", cfg.AutoStartBridge, func(checked bool) {
		cfg.AutoStartBridge = checked
	})
	form.AddInputField("Bridge Command", cfg.BridgeCommand, 128, nil, func(text string) {
		cfg.BridgeCommand = strings.TrimSpace(text)
	})
	form.AddInputField("Bridge Args (comma)", strings.Join(cfg.BridgeArgs, ","), 128, nil, func(text string) {
		cfg.BridgeArgs = splitCSV(text)
	})
	form.AddInputField("Bridge Work Dir", cfg.BridgeWorkDir, 256, nil, func(text string) {
		cfg.BridgeWorkDir = strings.TrimSpace(text)
	})
	addIntField(form, "Bridge Startup Timeout (sec)", cfg.BridgeStartupTimeout, func(value int) {
		cfg.BridgeStartupTimeout = value
	})
	addAllowFromField(form, &cfg.AllowFrom)
	return wrapWithBack(form, s)
}
...
func addAllowFromField(form *tview.Form, allowFrom *freeclawconfig.FlexibleStringSlice) {
...
func splitCSV(input string) freeclawconfig.FlexibleStringSlice {
