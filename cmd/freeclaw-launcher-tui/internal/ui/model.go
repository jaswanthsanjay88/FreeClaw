...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					freeclawconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model freeclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []freeclawconfig.ModelConfig) {
...
func isModelValid(model freeclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *freeclawconfig.ModelConfig) {
	if model == nil {
		return
	}
