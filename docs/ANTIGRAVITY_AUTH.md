...
10. [Creating a New Provider in PicoClaw](#creating-a-new-provider-in-freeclaw)
...
Set credentials through environment variables instead of embedding them in source code:

```bash
FREECLAW_GOOGLE_OAUTH_CLIENT_ID=your-client-id
FREECLAW_GOOGLE_OAUTH_CLIENT_SECRET=your-client-secret
```
...
Auth profiles are stored in `~/.freeclaw/auth.json`:
...
If your provider requires OAuth or special authentication, add a case to `cmd/freeclaw/cmd_auth.go`:
...
freeclaw auth login --provider your-provider

# List models (for Antigravity)
freeclaw auth models

# Start the gateway
freeclaw gateway

# Run an agent with a specific model
freeclaw agent -m "Hello" --model your-model
```
...
export FREECLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export FREECLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `freeclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.freeclaw/auth.json`
- Re-run `freeclaw auth login --provider antigravity`
