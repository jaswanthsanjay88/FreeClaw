...
```bash
freeclaw auth login --provider antigravity
```
...
### Switch Models
You can change the default model in `~/.freeclaw/config.json` or override it via the CLI:

```bash
# Override for a single command
freeclaw agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `FREECLAW_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.freeclaw/auth.json user@your-server:~/.freeclaw/
    ```
...
*   **404 Not Found**: Ensure you are using a model ID from the `freeclaw auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
